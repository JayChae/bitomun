...
export async function POST(request: NextRequest) {
  try {
    const data: OtherRequest = await request.json();
    ...
    const adminMailOptions = {
      ...
      subject: `[비토문] 새로운 기타 문의 - ${data.name}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #333;">새로운 기타 문의가 접수되었습니다.</h2>
          <div style="background-color: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
            ...
            <p style="margin: 10px 0;"><strong>문의 시간:</strong> ${new Date().toLocaleString("ko-KR", { timeZone: "Asia/Seoul" })}</p>
          </div>
    ...
    const applicantMailOptions = {
      from: process.env.SMTP_FROM || process.env.SMTP_USER,
      to: data.email,
      subject: `[비토문] 문의가 접수되었습니다`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #333;">안녕하세요, ${data.name}님</h2>
          <p style="font-size: 16px; line-height: 1.6;">비토문 문의가 정상적으로 접수되었습니다.</p>

          <div style="background-color: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
            <h3 style="margin-top: 0; color: #f97316;">문의 내용</h3>
            <p style="margin: 10px 0;"><strong>이름:</strong> ${data.name}</p>
            <p style="margin: 10px 0;"><strong>이메일:</strong> ${data.email}</p>
            ${data.phone ? `<p style="margin: 10px 0;"><strong>전화번호:</strong> ${data.phone}</p>` : ""}
            <p style="margin: 10px 0;"><strong>문의 내용:</strong></p>
