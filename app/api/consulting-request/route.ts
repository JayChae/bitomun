...
export async function POST(request: NextRequest) {
  try {
    const data: ConsultingRequest = await request.json();
    ...
    const adminMailOptions = {
      ...
      subject: `[비토문] 새로운 컨설팅 신청 - ${data.name}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #333;">새로운 컨설팅 신청이 접수되었습니다.</h2>
          <div style="background-color: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
            <p style="margin: 10px 0;"><strong>이름:</strong> ${data.name}</p>
            <p style="margin: 10px 0;"><strong>전화번호:</strong> ${data.phone}</p>
            <p style="margin: 10px 0;"><strong>이메일:</strong> ${data.email}</p>
            <p style="margin: 10px 0;"><strong>문의 내용:</strong></p>
            <p style="margin: 10px 0; padding: 15px; background-color: white; border-left: 3px solid #f97316; white-space: pre-wrap;">${data.message || "(내용 없음)"}</p>
    ...
    const applicantMailOptions = {
      from: process.env.SMTP_FROM || process.env.SMTP_USER,
      to: data.email,
      subject: `[비토문] 컨설팅 신청이 접수되었습니다`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #333;">안녕하세요, ${data.name}님</h2>
          <p style="font-size: 16px; line-height: 1.6;">비토문 컨설팅 신청이 정상적으로 접수되었습니다.</p>
    ...
    // 콘솔 로그
    console.log("=== 새로운 컨설팅 신청 ===");
    console.log("이름:", data.name);
    console.log("전화번호:", data.phone);
    console.log("이메일:", data.email);
    console.log("문의 내용:", data.message || "(내용 없음)");
    ...
    console.error("컨설팅 신청 처리 중 오류:", error);
